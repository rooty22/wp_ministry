...
class HorizontalMenu extends AdminSettingsModel
{
	...
	{
		$admin_bar_mode = (array) AdminSettings::get_instance()->get();
		$admin_bar_mode = (!empty($admin_bar_mode['admin_bar_mode'])) ? $admin_bar_mode['admin_bar_mode'] : 'light';
		...
		foreach ($menu as $key => $item) {
			...
			if (isset($item[6]) && !empty($item[6])) {
				...
				if ('none' === $item[6] || 'div' === $item[6]) {
					...
					$img = '<img ' . esc_attr($imgstyle) . ' class="wp-menu-image" src="' . esc_url($item[6]) . '" alt="" />';
				}
			}
			...
			if ($horz_menu_type == 'icons_only') {
				$wp_adminify_menu .= '<li class="wp_adminify_toplevel navbar-item has-dropdown dropdown is-hoverable topmenu-' . esc_attr($id) . '" id="' . esc_attr($id) . '"><a ' . esc_attr($href) . ' ' . esc_attr($class) . '>' . wp_kses_post($img) . '</a>';
			} elseif ($horz_menu_type == 'text_only') {
				$wp_adminify_menu .= '<li class="wp_adminify_toplevel navbar-item has-dropdown dropdown is-hoverable topmenu-' . esc_attr($id) . '" id="' . esc_attr($id) . '"><a ' . esc_attr($href) . ' ' . esc_attr($class) . '><span class="compact">' . wp_kses_post($anchor) . '</span></a>';
			} elseif ($horz_menu_type == 'both') {
				$wp_adminify_menu .= '<li class="wp_adminify_toplevel navbar-item has-dropdown dropdown is-hoverable topmenu-' . esc_attr($id) . '" id=' . esc_attr($id) . '><a ' . esc_attr($href) . ' ' . esc_attr($class) . '>' . wp_kses_post($img) . ' ' . wp_kses_post($anchor) . '</a>';
			}

			// Sub level menus
			if (!empty($submenu[$item[2]])) {
				if (!isset($ulclass)) {
					$ulclass = 'navbar-dropdown dropdown-menu is-boxed';
				}
